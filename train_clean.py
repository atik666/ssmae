...
def track_pseudo_label_accuracy(pseudo_label_memory, unlabeled_dataset, epoch_new_pseudo_labels, correct_total_count, previous_total_count):
    """
    Track the number of correctly pseudo-labeled samples without re-evaluating all samples each time.
    
    Args:
        pseudo_label_memory: Dictionary mapping image paths to pseudo-labels
        unlabeled_dataset: Dataset containing the actual labels
        epoch_new_pseudo_labels: Number of new pseudo-labels added in this epoch
        correct_total_count: Running count of correct pseudo-labels across all epochs
        previous_total_count: Total number of pseudo-labels before this epoch
    
    Returns:
        new_correct_count: Updated running count of correct pseudo-labels
    """
    # Create a mapping from path to true class index
    path_to_class = {path: class_idx for path, class_idx in unlabeled_dataset.samples}
    
    # Only check the new pseudo-labels (the last epoch_new_pseudo_labels items)
    current_total = len(pseudo_label_memory)
    new_paths = list(pseudo_label_memory.keys())[previous_total_count:current_total]
    
    # Count correct new pseudo-labels
    correct_new = 0
    for img_path in new_paths:
        pseudo_label = pseudo_label_memory[img_path]
        true_label = path_to_class[img_path]
        if pseudo_label == true_label:
            correct_new += 1
    
    # Update the running total
    new_correct_count = correct_total_count + correct_new
    
    # Print only raw numbers
    print(f"New correct pseudo-labels this epoch: {correct_new}/{epoch_new_pseudo_labels}")
    print(f"Total correct pseudo-labels so far: {new_correct_count}/{len(pseudo_label_memory)}")
    
    return new_correct_count
...
def train_SSMAE_w_unlabeled(model, unlabeled_data_path, labeled_data_train_path, labeled_data_test_path, optimizer, device, 
                ...
                confidence_threshold=0.95, high_conf_acc_threshold=90.0, **kwargs):
    ...
    correct_pseudo_labels_count = 0  # Initialize counter for correctly pseudo-labeled samples
    previous_total_count = 0  # Initialize counter for total pseudo-labels from previous epochs
    ...
    for epoch in range(num_epochs):
        ...
        if epoch > 0:  # Skip first epoch as we don't have high_conf_accuracy yet
        ...
        # Track pseudo-label accuracy using raw numbers
        if epoch_new_pseudo_labels > 0:
            correct_pseudo_labels_count = track_pseudo_label_accuracy(
                pseudo_label_memory, unlabeled_dataset, epoch_new_pseudo_labels, 
                correct_pseudo_labels_count, previous_total_count
            )
            previous_total_count = len(pseudo_label_memory)
